...
public class StringUtil extends StringUtils {
    ...
    public static String getFIrstSixAndLastFourBankCardNo(String bankCardNo) {
        try {
            ...
            logger.error("getFIrstSixAndLastFourBankCardNo: param : {}, error : {}", new Object[]{bankCardNo, e.getMessage()});
            return "";
        }
    }
    ...
    /**
     * 参数中是否都不为空值，只要有一个为空，则返回false，全不为空则返回true
     *
     * @param params
     * @return
     */
    public static boolean hasNotBlank(Object... params) {
    ...
    /**
     * 数组数据校验(所有参数数组长度>0 且长度相同 返回true)
     *
     * @param objArray
     * @return
     */
    public static boolean checkArray(Object[]... objArray) {

        int expectedLength = 0;

        for (Object[] objArr : objArray) {
            if (null == objArr || objArr.length < 1) {
                return false;
            } else {
                if (expectedLength == 0) {
                    expectedLength = objArr.length;
                    continue;
                }

                if (expectedLength != 0 && expectedLength != objArr.length) {
                    return false;
                }
            }
        }
