...
//}
//在 TheClass 类定义的上面有一个@MyAnnotation 的注解。
...
// 就像之前我们的那个例子那样。
// 在注解定义中的两个指示@Retention(RetentionPolicy.RUNTIME)和@Target(ElementType.TYPE)，
// 说明了这个注解该如何使用。
// @Retention(RetentionPolicy.RUNTIME)表示这个注解可以在运行期通过反射访问。
// 如果你没有在注解定义的时候使用这个指示那么这个注解的信息不会保留到运行期，这样反射就无法获取它的信息。
// @Target(ElementType.TYPE) 表示这个注解只能用在类型上面（比如类跟接口）。
// 你同样可以把Type改为Field或者Method，或者你可以不用这个指示，
// 这样的话你的注解在类，方法和变量上就都可以使用了。
// 关于 Java 注解更详细的讲解可以访问 Java Annotations tutorial。
...
//  }
//}
//需要注意的是 Method.getParameterAnnotations()方法返回一个注解类型的二维数组，
// 每一个方法的参数包含一个注解数组。
