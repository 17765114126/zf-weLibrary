 ...
 * @date 2020-{MONTH}-{DAY}
 */
public class Socket通信中由read返回值造成的的死锁问题 {
//示例
...
//}
//运行结果如下：
    //https://wiki.jikexueyuan.com/project/java-socket/images/result.jpg
    //https://wiki.jikexueyuan.com/project/java-socket/images/result1.jpg

//问题的引出
//明确问题

//客户端与服务器端在接收和发送数据时，read()和write()方法不一定要对应，比如，其中一方可以一次发送多个字节的数据，而另一方可以一个字节一个字节地接收，也可以一个字节一个字节地方送，而多个字节多个字节地接收。
...
//    totalBytesRcvd += bytesRcvd;
//}  // data array is full
//客户端从 Socket 套接字中读取数据，直到收到的数据的字节长度和原来发送的数据的字节长度相同为止，这里的前提是已经知道了要从服务器端接收的数据的大小，如果现在我们不知道要反馈回来的数据的大小，那么我们只能用 read 方法不断读取，直到 read()返回 -1，说明接收到了所有的数据。
// 我这里采用一个字节一个字节读取的方式，代码改为如下：
...
//}
//这时问题就来了，输出结果如下：
    //https://wiki.jikexueyuan.com/project/java-socket/images/result2.jpg
    //https://wiki.jikexueyuan.com/project/java-socket/images/result3.jpg
//问题的分析
...
//}
//此时运行结果如下：
    //https://wiki.jikexueyuan.com/project/java-socket/images/result4.jpg
    //https://wiki.jikexueyuan.com/project/java-socket/images/result5.jpg
...
//那么，如果在客户端不知道反馈回来的数据的情况下，该如何避免死锁呢？
// Java 的 Socket 类提供了 shutdownOutput()和 shutdownInput()两个方法，用来分别只关闭 Socket 的输出流和输入流，而不影响其对应的输入流和输出流，那么我们便可以在客户端发送完数据后，调用 shutdownOutput()方法将套接字的输出流关闭，这样，服务端的 read()方法便会返回 -1，继续往下执行，最后关闭服务端的套接字，而后客户端的 read()()方法也会返回 -1，继续往下执行，直到关闭套接字。
...
//这样，便得到了预期的运行结果，如下：
    //https://wiki.jikexueyuan.com/project/java-socket/images/result6.jpg
    //https://wiki.jikexueyuan.com/project/java-socket/images/result7.jpg
//总结
