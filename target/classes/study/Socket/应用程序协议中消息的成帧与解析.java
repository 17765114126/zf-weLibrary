 ...
 * @date 2020-{MONTH}-{DAY}
 */
public class 应用程序协议中消息的成帧与解析 {
//程序间达成的某种包含了信息交换的形式和意义的共识称为协议，用来实现特定应用程序的协议叫做应用程序协议。
// 大部分应用程序协议是根据由字段序列组成的离散信息定义的，
// 其中每个字段中都包含了一段以位序列编码
// （即二进制字节编码，也可以使用基于文本编码的方式，但常用协议如：TCP、UDP、HTTP 等在传输数据时，都是以位序列编码的）的特定信息。
...
//基于定界符：消息的结束由一个唯一的标记指出，即发送者在传输完数据后显式添加的一个特定字节序列，这个特殊标记不能在传输的数据中出现（这也不是绝对的，应用填充技术能够对消息中出现的定界符进行修改，从而使接收者不将其识别为定界符）。
// 该方法通常用在以文本方式编码的消息中。
...
//  }
//}
//下面的代码实现了基于长度的成帧方法，适用于长度小于 65535 个字节的消息。
// 发送者首先给出指定消息的长度，并将长度信息以 big-endian 顺序（从左边开始，由高位到低位发送）存入 2 个字节的整数中，再将这两个字节存放在完整的消息内容前，连同消息一起写入输出流；
// 在接收端，使用 DataInputStream 读取整型的长度信息，readFully（）方法将阻塞等待，直到给定的数组完全填满。
